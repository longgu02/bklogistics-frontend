...
export default function OrderSecondStep() {
  ...
  >([]);
  const [newRequireMaterial, setNewRequireMaterial] = React.useState<RequireMaterial[] | undefined>([]);
  ...
  getAllMaterialOnChain(5).then((result) => {
    // console.log("🚀 ~ file: OrderSecondStep.tsx:73 ~ getAllMaterialOnChain ~ result:", result.materials);
  ...
  //   console.log(
  //     "🚀 ~ file: OrderSecondStep.tsx:71 ~ OrderSecondStep ~ MaterialList:",
  //     MaterialList
  //   );
  // const productList: Product[] = getProductList(pId);
  const productList: Product[] = [];
  getAllProductOnChain(5).then((result) => {
  ...
  //   console.log(
  //     "🚀 ~ file: OrderSecondStep.tsx:88 ~ OrderSecondStep ~ productList:",
  //     productList
  //   );
  ...
  const handleAddNewRqMaterial = () =>{
    ...
    if(signer && newRequireMaterial && product){
      setIsLoading(true);
      const {addRequiredMaterial, contract} = useProductContract(signer, chainId);
      newRequireMaterial.forEach(async(material) => {
        const res = await addRequiredMaterial(Number(product.id), material.materialId, material.quantity, material.unit);
        await res.wait();
      });
      setIsLoading(false);
      dispatch(_addProduct(product));
  ...
  let newRqMaterial: RequireMaterial[] = [];
  const OPT1 = () => {
    ...
    const handleCreateRqMaterial = () => {
      if (material && material.materialId) {
        ...
        console.log(
          "🚀 ~ file: OrderSecondStep.tsx:186 ~ handleCreateRqMaterial ~ newRqMaterial:",
          newRqMaterial
          );
    ...
    const handleDeleteMaterial = (materialId: number) => {
      ...
      console.log(
        "🚀 ~ file: OrderSecondStep.tsx:194 ~ handleDeleteMaterial ~ newRqMaterial:",
        newRqMaterial
      );
      setNewRequireMaterial(newRqMaterial);
    };
  ...
  return (
    <BaseStepper isDisabled={!product} handleConfirm={isNewProduct? handleAddNewRqMaterial : handleAddProduct} isLoading={isLoading}>
      <OPT1 />
    </BaseStepper>
  );
}
